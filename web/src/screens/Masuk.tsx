...
export function Masuk({ onMasuk }: { onMasuk: () => void }) {
  const [subjectId, setSubjectId] = useState('u.cfo');
  ...
  const kirim = (e: React.FormEvent) => {
    ...
    setSibuk(true);
    setGalat('');
    void api.login(subjectId.trim(), sandi)
      .then(async (r) => {
        saveSession({ token: r.token, user: subjectId.trim() });
        // companyId harus diketahui sebelum KPI bisa diminta. Menyimpan token
        // saja membuat dashboard masuk tanpa pernah memanggil endpoint KPI.
        const ctx = await api.context();
        if (ctx.companies[0]) saveSession({ companyId: ctx.companies[0].id });
        onMasuk();
      })
  ...
  return (
    <div className="min-h-screen grid place-items-center bg-navy-50 p-6">
      <form onSubmit={kirim} className="w-full max-w-[420px] rounded-2xl border border-line bg-white p-8 shadow-sm">
        <h1 className="text-[22px] font-bold text-navy-800">Nalarasa OS</h1>
        <p className="mt-1 text-[13px] text-ink-400">Dashboard pemilik · masuk untuk melihat angka outlet Anda</p>
