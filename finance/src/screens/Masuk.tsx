...
/**
 * Gerbang masuk aplikasi keuangan.
 *
 * Angka contoh yang tampil seperti angka nyata adalah masalah paling serius
 * di aplikasi keuangan: keputusan pajak dan pembayaran diambil darinya.
 * Ditahan di depan pintu.
 */
export function Masuk({ onMasuk }: { onMasuk: () => void }) {
  const [subjectId, setSubjectId] = useState('u.controller');
  ...
  return (
    <div className="min-h-screen grid place-items-center bg-navy-50 p-6">
      <form onSubmit={kirim} className="w-full max-w-[420px] rounded-2xl border border-line bg-white p-8 shadow-sm">
        <h1 className="text-[22px] font-bold text-navy-800">Keuangan</h1>
        <p className="mt-1 text-[13px] text-ink-400">Nalarasa OS · masuk untuk melihat buku besar Anda</p>
